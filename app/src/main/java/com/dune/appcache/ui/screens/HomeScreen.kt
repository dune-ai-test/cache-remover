    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.fillMaxSize()) {
            LazyColumn(
                ...
                if (!uiState.hasUsageAccess) {
                    item { UsageAccessCard(onGrant = { CacheActions.openUsageAccessSettings(context) }) }
                }
        ...
        if (uiState.hasUsageAccess && uiState.apps.isNotEmpty()) {
            Button(
                ...
                Icon(Icons.Rounded.DeleteSweep, contentDescription = null, tint = androidx.compose.ui.graphics.Color.White)
                Spacer(Modifier.width(8.dp).height(0.dp))
                Text("Remove All", color = androidx.compose.ui.graphics.Color.White, style = MaterialTheme.typography.bodyLarge)
            }
        }
    }
...
private fun UsageAccessCard(onGrant: () -> Unit) {
    Column(
        ...
        Text(
            "Android requires a one-time \"Usage access\" permission before any app can read how much cache other apps are using.",
            style = MaterialTheme.typography.bodyMedium,
            color = TextSecondary,
        ...
        Button(
            ...
            Icon(Icons.Rounded.CleaningServices, contentDescription = null, tint = androidx.compose.ui.graphics.Color.White)
            Spacer(Modifier.width(8.dp).height(0.dp))
...
private fun AppRow(app: AppCacheInfo, onRemove: () -> Unit) {
    ...
    Row(
        ...
        Row(
            ...
            Spacer(Modifier.width(6.dp).height(0.dp))
            Text(
    ...
    androidx.compose.material3.AlertDialog(
        ...
        confirmButton = {
            Button(onClick = onOpenStorageSettings, colors = ButtonDefaults.buttonColors(containerColor = AccentGreen)) {
                Text("Open Storage Settings", color = androidx.compose.ui.graphics.Color.White)
            }
        },
        dismissButton = {
            Button(onClick = onDismiss, colors = ButtonDefaults.buttonColors(containerColor = androidx.compose.ui.graphics.Color.Transparent)) {
